...
StyleDictionary.registerFilter({
  ...
  matcher: function (token) {
    ...
    const isPrimitive = token.original.value && token.original.value.startsWith("#");
...
StyleDictionary.registerFilter({
  ...
  matcher: function (token) {
    const isColor = token.type === "color" || token.path[0] === "color";
    const isSemantic = token.original.value && !token.original.value.startsWith("#");
      ...
      .map(([category, tokens]) => {
        ...
        const variables = tokens
          .map((token) => {
            ...
            const referencedPath = token.original.value.replace("{color.", "").replace(".value}", "").split(".");
...
StyleDictionary.registerFormat({
  ...
  formatter: function ({ dictionary }) {
    ...
    dictionary.allTokens.forEach((token) => {
      ...
      // Special handling for nested categories like accent-sage
      if (path[1] === 'accent') {
        category = `${path[1]}-${path[2]}`; // e.g., 'accent-sage'
      } else {
